...
#define SSID	"HONOR 90 Lite"
#define PASSWORD "12345678"

uint8_t RemoteIP[] = {192,168,35,78};
    ...
    if(Socket != -1)
    {
      ...
      if(ret == WIFI_STATUS_OK)
      {
        if(Datalen > 0)
        {
          ...
          //TERMOUT("\n%s\n",TxData);
          //uint8_t* acc_data;
//          acc_data[1] = pDataXYZ[0] >> 8;
//          acc_data[0] = pDataXYZ[0] & 0xff;
          //acc_data = (uint8_t)pDataXYZ;
          //&acc_data[0] = (uint8_t*)&pDataXYZ[0];
          //TERMOUT("  %d  ", *acc_data);
